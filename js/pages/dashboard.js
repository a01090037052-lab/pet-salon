...
App.pages.dashboard = {
  async render(container) {
    try {
      ...
      const activePromotions = promotions.filter(p => p.isActive && p.startDate <= today && p.endDate >= today);
      ...
      // Active promotions banner HTML
      const promoBannerHtml = activePromotions.length > 0 ? `
        <div style="background:linear-gradient(135deg,#6366F1,#8B5CF6);border-radius:var(--radius-xl);padding:16px 20px;margin-bottom:16px;color:#fff">
          <div style="font-weight:800;margin-bottom:6px">&#x1F389; 현재 진행 중인 프로모션</div>
          ${activePromotions.map(p => {
            const discountText = p.discountType === 'percent' ? p.discountValue + '% 할인' : App.formatCurrency(p.discountValue) + ' 할인';
            return `<div style="display:flex;align-items:center;gap:12px;padding:8px 0;border-top:1px solid rgba(255,255,255,0.2)">
              <div style="flex:1">
                <strong>${App.escapeHtml(p.name)}</strong> - ${discountText}
                <div style="font-size:0.8rem;opacity:0.8">${p.startDate} ~ ${p.endDate}</div>
              </div>
              <button class="btn btn-sm btn-promo-sms" data-id="${p.id}" style="background:rgba(255,255,255,0.2);color:#fff;border:1px solid rgba(255,255,255,0.3);font-weight:600">문자 발송</button>
            </div>`;
          }).join('')}
        </div>
      ` : '';
      ...
      container.innerHTML = `
        ...
        <div class="card" style="margin-bottom:20px">
          <div class="card-header">
            <span class="card-title">&#x2702; 이번 달 서비스별 매출</span>
          </div>
          <div class="card-body" style="padding:16px">
        ...
        <div class="card" style="margin-top:20px">
          <div class="card-header">
            <span class="card-title">&#x1F4B8; 미수금(외상) 현황</span>
  ...
  async init() {
    // Promo SMS buttons
    document.querySelectorAll('.btn-promo-sms').forEach(btn => {
      btn.addEventListener('click', () => {
        const promoId = Number(btn.dataset.id);
        App.pages.settings?.showPromoSmsModal(promoId);
      });
    });
